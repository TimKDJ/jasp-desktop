...
.messages <- function(class, type) {
  ...
  m$error$packageException <- 
  "This analysis terminated after we called the R package {{package}}.<br><br>We received the following message:<br>{{error}}<br><div class=stack-trace-selector><span>Stack trace</span><div class=stack-trace-arrow></div></div><div class=stack-trace>{{stackTrace}}</div><br>Perhaps we can assist you with this problem, please report the message above at: https://jasp-stats.org/bug-reports"
  m$error$jaspException <- 
